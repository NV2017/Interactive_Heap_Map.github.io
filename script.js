...
// Un-comment the below till 'update();' to get animated background
...
/* Combined scroll navbar hide and header fixed*/
...
const legendColors = ['steelblue', 'lightsteelblue', 'orange', 'crimson'];
const legendTextList = ["<=-1","<=0","<=1",">1"];
...
let baseTemperature;
let monthlyVariance = [];
let rawData = fetch('https://raw.githubusercontent.com/freeCodeCamp/ProjectReferenceData/master/global-temperature.json')
  .then(response => response.json())
  .then(response => {
    baseTemperature = response.baseTemperature;
    monthlyVariance = response.monthlyVariance;

		let subHeading = document.getElementById("description")
		subHeading.innerText += " " + baseTemperature + "℃";
		subHeading.style.fontSize = "100%";
    ...
    generateScales(monthlyVariance);

    generateAxes(svg_element,height,padding,width);

    drawCells(svg_element,monthlyVariance,baseTemperature);

    // let legend_element = d3.select("#legend")
    generateLegend(svg_element,legendColors);
});
...
let generateScales = (monthlyVariance) => {
  let allYears = monthlyVariance.map(d => d['year']);

  // xScale deals with the position on web page
  xScale = d3.scaleLinear()
             .range([padding,(width-padding)]) // This is position bounds on the horizontal side of the page
             .domain([d3.min(allYears),d3.max(allYears)+1]) // This is limit of real values form both the sides of the axis

  yScale = d3.scaleTime()
            // .domain([1,12]) // This is limit of real values form both the sides of the axis
            .domain([new Date('1970-01-01T00:00:00Z'),new Date('1970-12-01T00:00:00Z')])
            .range([(height-padding),padding]) // This is position bounds on the page
}

let generateAxes = (svg_element,height,padding) => {
  // Defining type and scale of the x-axis
  let xAxis = d3.axisBottom(xScale) // Returns a set of elements
                .tickFormat(d3.format('d'));

  // Rendering x-axis on the HTML page using elements from 'xAxis'
  svg_element.append('g')
             .call(xAxis)
             .attr("id","x-axis")
             .attr("transform","translate(0,"+(height-padding)+")")

  let yAxis = d3.axisLeft(yScale); // Returns a set of elements

  // Rendering y-axis on the HTML page using elements from 'yAxis'
  svg_element.append('g')
             .call(yAxis)
             .attr("id","y-axis")
...
let drawCells = (svg_element,monthlyVariance,baseTemperature) => {
	...
	svg_element.selectAll('rect')
             ...
             .style('fill', item => {
              ...
              .attr('y',item=>yScale(new Date(1970,item['month'],1,0,0,0,0))) //new Date('1970-01-01T00:00:00Z')
              .attr('x',item=>xScale(item['year']+0.5))
              .attr('width',"0.16rem")
              .attr('height',"1.85rem")
					      ...
					      div.transition()
					        .duration(100)
					        .style("opacity", 0.5);
					      div.html("<span class='year'>" + d.year + " - " + monthNames[d.month - 1] + "</span><br>" +
					          "<span class='temperature'>" + (Math.floor((d.variance + baseTemperature) * 1000) / 1000) + " ℃" + "</span><br>" +
...
let generateLegend = (svg_element) => {
  svg_element.append(d3.creator("g"))
             ...
             .append('rect')
             .attr('y',"30rem")
             .attr('x',(item,index)=>10+index*3+"rem")
	 ...
	 svg_element.append("g")
							...
							.append('text')
							.attr('y',"32rem")
							.attr('x',(item,index)=>11+index*3+"rem")
